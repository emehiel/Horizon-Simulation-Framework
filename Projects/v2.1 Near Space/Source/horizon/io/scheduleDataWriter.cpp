...
bool scheduleDataWriter::writeAll(const list<systemSchedule*> schedules, System* system)
{
	...
	for(size_t i = 1; i <= schedulesToShow; i++, sIt++) {
		...
		{			
			...
			// Changed code so that dynamic assets only write out state data...this is because pos and vel
			// are contained in the state data of these types.
			if (((*assIt)->getPos()->getType() != POSITION_TYPE_DYNAMIC_LLA) && ((*assIt)->getPos()->getType() != POSITION_TYPE_DYNAMIC_ECI))
			{
