...
public:
	 ...
	 */
	Profile<T>(const Profile<T>& otherProf) : data(otherProf.data) {}

	/**
	 * Creates a new Profile using an initial point
	 * @param pointIn the initial point in the profile
	 */
	Profile<T>(const pair<double, T>& pointIn) { data.insert(pointIn); }
...
//		_ASSERTE(data.size()>0);
	...
	*
	* If the integral falls below the lower limit, the value fo the integral is set to the lower limit
	*
	* @param start the start of the integral
	* @param end the end of the integral
	* @param saveFreq the frequence to evaluate the integral
	...
	*/
	...
	Profile<T>& operator+=(const Profile &rhs) {
		...
		for(lIt= uniquetimes.begin(); lIt != uniquetimes.end(); lIt++)
		{
			...
			if(dataIt == data.begin()) { profOut.push_back(*lIt, (*(--rhsIt)).second); }
			else if(rhsIt == rhs.begin()) { profOut.push_back(*lIt, (*(--dataIt)).second); }
			else { profOut.push_back(*lIt, (*(--dataIt)).second + (*(--rhsIt)).second); }
		}
		data = profOut.data;
		return *this;
	}
