...
namespace Hsf.Utility
{
    public class XqlParser
    {
        ...
        {
            // Loads raw xml from string into content
            _xmlContent = raw;
        }
        ...
        {
            // Supported:
            //   SELECT [attribute names, keywords] FROM [element types]
            //   keywords include: _innerXml, _outerXml, *
            ...
            try
            {
                xDoc.LoadXml("<_root>" + _xmlContent + "</_root>");
            }
            catch (XmlException)
            {
            ...
            if (match.Success)
            {
                // Get specific attributes and elements
                string[] splitter = { ", " };
        ...
        {
            // Takes an XmlElement and returns a hashtable with the attributes specified in typeString. Also supports
            // the _innerXml and _outerXml keyword.
            ...
            else
            {
                ...
                foreach (string type in types)
                {
                    if (type == "_innerXml")
                    {
                        toReturn.Add("_innerXml", element.InnerXml);
                    }
                    else if (type == "_outerXml")
                    {
                        toReturn.Add("_outerXml", element.OuterXml);
                    }
                    else
                    {
                        try
