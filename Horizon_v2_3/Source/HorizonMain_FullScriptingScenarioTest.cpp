...
int main( int argc, char *argv[ ], char *envp[ ] ) {
	...
	string logFileName = argv[5];
	double endTime = atof(argv[6]);
	bool mtSched = atob(argv[7]); // 1 for MT, 0 for ST
	int nThread = atoi(argv[8]);

	// ***** MODIFICATIONS *****
	// 1. Overwrite SimEnd Time
	// 2. Switch for scheduler
	// 3. Log file
	// *************************
	...
	if(mtSched) {
		systemScheduler = new MultiThreadedScheduler(nThread);
	} else {
	...
	// Write out log file
	// Write log file
	FILE* fid = fopen(logFileName.c_str(), "a");
	fprintf(fid, "%d, %d, %g, %d, %d, %d, %d\n",
		(int)mtSched, nThread, endTime,
