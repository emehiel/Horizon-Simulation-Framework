...
Matrix cumprod(const Matrix& A)
{
	Matrix C(A.getNumRows(), A.getNumCols(), 0.0);

	deque<double> AData = A.getData();
	deque<double> CData = C.getData();

	deque<double>::iterator iter1;

	if( A.getNumCols() || A.getNumRows()){
		// Constructing a vector of partial products
		int k = 0;
		double ppartotal = 0.0;
		for (iter1 = AData.begin(); iter1 != AData.end(); iter1++){
			ppartotal = accumulate(AData.begin(), iter1 + 1, 1.0, multiplies<double>());
			CData[k] = ppartotal;
			k++;
		}
	}
