...
namespace lua {
	// Global variable to hold LUA state
	lua_State* L;
}
...
int main( int argc, char *argv[ ], char *envp[ ] ) {
	...
	simParams::setSimEnd(simEndTime); // Update simulation end time
	...
	//systemScheduler->nThread = nThread; // Set number of threads
	...
	if(nSchedEval != 0) {
		...
		systemEvaluator = horizon::util::adapt::createScheduleEvaluator(L, modelInputXMLNode.getChildNode("SCHEDULE_EVALUATOR"));
	}
	else {
		// If node doesnt exist, pass empty node in and the 
		// default Schedule Evaluator will be used
		systemEvaluator = createScheduleEvaluator(L, XMLNode::emptyXMLNode);
	}
	...
	// Write log file
	const char* logFileName = "MT_Runs.log";
	FILE* fid = fopen(logFileName, "a");
	fprintf(fid, "%d, %g, %d, %d, %d, %d\n",
		nThread, simEndTime,
		systemScheduler->totalTimeMs,
		systemScheduler->pregenTimeMs,
		systemScheduler->schedTimeMs,
		systemScheduler->accumSchedTimeMs);
